...
export async function callAgent(client: MongoClient, query: string, thread_id: string) {
  try {
    ...
    // Define the state structure for the agent workflow (simplified types)
    const GraphState = Annotation.Root({
      messages: Annotation<BaseMessage[]>({
        // Reducer function: how to combine old and new messages
        reducer: (x: BaseMessage[], y: BaseMessage[]) => x.concat(y), // Simply concatenate old messages (x) with new messages (y)
      }),
    })

    // Create a custom tool for searching furniture inventory
    const itemLookupFunction = async (params: { query: string; n?: number }) => {
      const { query, n = 10 } = params
      try {
    ...
    // Create tool with type casting to avoid complex inference
    const itemLookupTool = (tool as any)(itemLookupFunction, {
      name: "item_lookup",
